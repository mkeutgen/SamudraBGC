...
class PerturbConfig:
    ...
    pert_std_temp: float = 0.05  # °C additive

    # BGC (log-space sigma; ~relative std for small sigma)
    pert_rel_dic: float = 0.02
    pert_rel_o2: float = 0.02
    pert_rel_no3: float = 0.02
    ...
    # ensemble members
    ensembles: tuple = tuple(f"ENS0{i}" for i in range(1, 11)) # ENS01 to ENS10
...
def perturb_restart(ens_name: str) -> dict:
    ...
    file_main = restart_dir / "MOM.res_Y2010_D001_S00000.nc"
    file_aux = restart_dir / "MOM.res_Y2010_D001_S00000_1.nc"
    ...
    with nc4.Dataset(file_main, "r+") as ds:
        ...
        for k in top_k:
            ...
            # Keep nonzero basin-mean anomalies so ensemble spread is visible in means
            ...
            o2[0, k, :, :] = apply_lognormal_multiplicative(
                ...
                eps_unit=diss_unit,
                enforce_mean_unity=False,  # allow small basin-mean offset for ensemble spread
            )
