#!/usr/bin/env python3
"""
Figure 2 v2 — Champion Model BGC Performance  (Laure feedback, 2026-04)
=========================================================================
Generates separate panel PNGs so Laure can pick and assemble combinations.

Outputs in figures/fig02_v2/:
...
Usage:
    python code_paper/fig02_v2.py
    sbatch code_paper/fig02_v2.sh
"""
...
OUTPUT_DIR = Path(__file__).resolve().parent / "figures" / "fig02_v2"
...
def plot_dic_zonal_mean(gt_zonal, pred_zonal, lat, output_dir):
    ...
    for cmap_name, cmap in COLORMAPS_DIC:
        ...
        # pcolormesh(X, Y, C): X=lat (nlat,), Y=depth (47,), C.T=(47, nlat)
        im = ax_c.pcolormesh(lat, depth_arr, gt_disp.T,
        ...
        ax_d.pcolormesh(lat, depth_arr, pred_disp.T,
                        cmap=cmap, vmin=vmin, vmax=vmax, shading="auto")
        ax_d.set_ylim(500, 0)
        ...
        cbar = fig.colorbar(im, ax=[ax_c, ax_d], shrink=0.65, pad=0.02,
