#!/bin/bash
# Champion model on coarsened data (0.25°, 5-day resolution)
# Same architecture as champion but on spatially/temporally coarsened data

#SBATCH --job-name=coarse_champ
...
#SBATCH --output=logs/coarsened_champion_train_%j.out
#SBATCH --error=logs/coarsened_champion_train_%j.err
...
CONFIG=configs/train/coarsened_0p25deg_5day_champion.yaml

echo "Training champion model on coarsened data (0.25°, 5-day)"
echo "Config: ${CONFIG}"
