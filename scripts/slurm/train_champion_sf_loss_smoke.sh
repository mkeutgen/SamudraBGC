#!/bin/bash
# Structure-Function Loss experiment (Snider & Bodner)
# Champion (MAE + gradient 0.10) + scalar structure-function penalty on psi/phi

#SBATCH --job-name=champ_sf_smoke
...
#SBATCH --output=logs/champion_sf_loss_smoke_%j.out
#SBATCH --error=logs/champion_sf_loss_smoke_%j.err
...
CONFIG=configs/train/champion_sf_loss_smoke.yaml

echo "Training champion_sf_loss"
echo "Loss: MAE + gradient 0.10 + structure-function (psi/phi, S2)"
echo "Config: ${CONFIG}"
