#!/bin/bash
# Champion Model - History 2
# Same as champion_model but with hist=2 (2 timesteps of history input)

#SBATCH --job-name=champ_hist2
...
#SBATCH --output=logs/champion_model_hist2_train_%j.out
#SBATCH --error=logs/champion_model_hist2_train_%j.err
...
CONFIG=configs/train/champion_model_hist2.yaml

echo "Training champion_model_hist2"
echo "Config: ${CONFIG}"
echo "Using $WORLD_SIZE GPUs across $SLURM_NNODES nodes"
echo "hist=2 (2 timesteps of history)"
