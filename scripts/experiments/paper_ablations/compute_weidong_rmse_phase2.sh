...
#SBATCH --output=logs/paper_ablations/weidong_rmse_phase2_%j.out
#SBATCH --error=logs/paper_ablations/weidong_rmse_phase2_%j.err
...
mkdir -p logs/paper_ablations

echo "Starting Weidong RMSE computation (phase2_helmholtz_grad010, ckpt_35)"
echo "Config: configs/eval/paper_ablations/phase2_helmholtz_grad010_eval_weidong.yaml"
echo "IC dates: scripts/ic_dates.npy (288 dates)"
echo "Lead times: 20"
echo ""

python scripts/compute_weidong_rmse.py \
    --config configs/eval/paper_ablations/phase2_helmholtz_grad010_eval_weidong.yaml \
