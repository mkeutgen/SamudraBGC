...
#SBATCH --output=logs/paper_ablations/phase1_vs_phase15_comparison_%j.out
#SBATCH --error=logs/paper_ablations/phase1_vs_phase15_comparison_%j.err
...
if [ ! -d "outputs/phase15_helmholtz_log_eval/predictions.zarr" ]; then
    ...
    srun --ntasks=1 \
         ...
         --gpus-per-node=1 \
         python -m ocean_emulators.eval \
         configs/eval/paper_ablations/phase15_helmholtz_log_eval.yaml
else
    echo "  ✓ Predictions already exist, skipping"
fi
...
python scripts/compare_rollouts.py \
    --config configs/eval/paper_ablations/phase1_vs_phase15_comparison.yaml \
