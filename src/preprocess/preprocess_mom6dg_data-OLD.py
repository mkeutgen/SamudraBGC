...
def parse_arguments():
    ...
    parser.add_argument(
        "--chunk-time", type=int, default=30, help="Chunk size for time dimension"
    )
    parser.add_argument(
        "--validate-only",
        action="store_true",
...
def process_mom6_cobalt_data(
    ...
    for y in years:
        ...
        ds_year.astype("float32").to_zarr(
            yearly_path, mode="w", consolidated=False, zarr_version=2, encoding=encoding
        )
