...
def merge_yearly_zarr_files(
    input_dir: Path,
    output_file: str = "bgc_data.zarr",
    time_chunk: int = 365,
    lev_chunk: int = 50,
    y_chunk: int = 68,
    x_chunk: int = 45,
    ...
    for v in combined.data_vars:
        if not np.issubdtype(combined[v].dtype, np.datetime64) and not np.issubdtype(
            combined[v].dtype, np.timedelta64
        ):
    ...
    combined.to_zarr(
        output_path, mode="w", consolidated=False, zarr_version=2, encoding=encoding
    )
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        print(
            "Usage: python merge_yearly_zarr.py <data_directory> [time_chunk] [lev_chunk] [y_chunk] [x_chunk]"
        )
    ...
    merge_yearly_zarr_files(
        data_dir, "bgc_data.zarr", time_chunk, lev_chunk, y_chunk, x_chunk
    )
